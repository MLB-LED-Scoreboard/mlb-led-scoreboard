...
class Dates:
  ...
  def next_important_date(self):
    today = datetime.today()
    return min(self.important_dates, key=lambda date:
      date["date"] - today if (date["date"] - today).days > 0 else timedelta.max)

  def __parse_important_dates(self, dates):
    self.important_dates = []
    self.__add_date("2020-02-10T00:00:00",     "pitchers and catchers report")
    self.__add_date("2020-02-21T00:00:00",     "Spring Training begins")
  ...
  def __add_date(self, date, text, max_days_to_count=999):
    if date != '':
      self.important_dates.append({"text": text, "date": datetime.strptime(date, MLB_DATE_FORMAT), "max_days": max_days_to_count})
