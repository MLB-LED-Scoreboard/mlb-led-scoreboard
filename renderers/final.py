...
class Final:
    ...
    def __render_scroll_text(self):
        ...
        return ScrollingText(
            self.canvas, coords["x"], coords["y"], coords["width"], font, color, self.bgcolor, scroll_text
        ).render(self.scroll_pos)
    ...
    def __render_final_inning(self):
        ...
        if self.data.config.layout.state_is_nohitter():
            ...
            graphics.DrawText(
                self.canvas, nohit_font["font"], nohit_coords["x"], nohit_coords["y"], nohit_color, nohit_text
            )
