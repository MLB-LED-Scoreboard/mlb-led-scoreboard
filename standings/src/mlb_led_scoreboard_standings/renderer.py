    ...
    def render(
        ...
        if data.is_postseason():
            render_bracket(
                ...
                self.layout,
                self.colors,
                data.leagues[self.standings_league],
            )
        else:
            render_standings(
                canvas,
        ...
        if data.is_postseason():
            if self.update % 20 == 0:
                if self.standings_league == "NL":
                    ...
                    self.standings_league = "NL"
        elif canvas.width == 32 and self.update % 5 == 0:
            if self.standings_stat == "w":
                self.standings_stat = "l"
            else:
                self.standings_stat = "w"
                data.advance_to_next_standings()
        elif canvas.width > 32 and self.update % 10 == 0:
            data.advance_to_next_standings()
...
def render_standings(
    ...
    if canvas.width > 32:
        __render_static_wide_standings(canvas, graphics, layout, colors, division, league)
    else:
        __render_rotating_standings(canvas, graphics, layout, colors, division, stat, league)


def __render_rotating_standings(canvas, graphics, layout, colors, division, stat, league):
    ...
    __render_standings_indicator(canvas, graphics, layout, colors, division, league)


def __render_standings_indicator(canvas, graphics, layout, colors, division, league):
    divider_color = get_standings_color_node(colors, "divider", league)
    team_name_color = get_standings_color_node(colors, "team.name", league)
...
def __render_static_wide_standings(canvas, graphics, layout, colors, division, league):
    coords = layout.coords("standings")
    font = layout.font("standings")
    divider_color = get_standings_color_node(colors, "divider", league)
    team_stat_color = get_standings_color_node(colors, "team.stat", league)
...
def render_bracket(canvas, graphics, layout, colors, league: League):
